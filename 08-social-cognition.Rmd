...
<iframe width="560" height="315" src="https://www.youtube.com/embed/VTNmLt7QX8E" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
...
Can we formalize this intuition?^[The following is based on [a model originally developed by Chris Lucas and his collaborators](https://journals.plos.org/plosone/article?id=10.1371/journal.pone.0092160).]
...
Remember that our goal is to *infer other people's preferences*, not predict their choices. So, after we see someone make a choice between some fruits, how do we infer what fruits they like best? We can invert Equation \@ref(eq:choicemodel) using Bayes's rule:
...
Both Rue and Jules pick Option 1, which includes a peach 🍑. Based on their choices alone, who do you think likes peaches more? (Or, at least, which person's choice would make you more confident they like peaches?)
...
Let's define a function that calculates the choice probability in Equation \@ref(eq:choicemodel). This is our likelihood function.
...
Unsurprisingly, Jules's choice has a much higher baseline probability.
...
[![](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/1Zk2mUWffNXrBd_9S_7FqD8KDCr9-CofY?usp=sharing)
...
```{r echo=FALSE, out.width='75%', fig.align='center', fig.cap='Cards used in the preference learning task from Jern, Lucas, & Kemp (2017).'}
knitr::include_graphics("images/08/cards.jpg")
```
...
They both choose the peach 🍑. The difference is that, for Rue, both fruits were placed in a bowl in front of her. For Jules, the peach was in a bowl in front her, and the banana was on a high shelf in another room.
...
```{r echo=FALSE, out.width='60%', fig.align='center', fig.cap='Example of a grid and an agent path from Jara-Ettinger et al (2020).'}
knitr::include_graphics("images/08/jara-ettinger-grid.png")
```
...
In the model, they use uniform priors over costs and rewards. Computing $p(\mathbf{a}|C,R)$ means just applying Equation \@ref(eq:optimalpolicy) repeatedly for each action the agent took in their sequence. (The researchers also modeled a notion of sub-goals that I've omitted.)
...
```{r echo=FALSE, out.width='90%', fig.align='center', fig.cap='Partial results from Experiment 1b of Jara-Ettinger et al (2020).'}
knitr::include_graphics("images/08/jara-ettinger-results.png")
```
